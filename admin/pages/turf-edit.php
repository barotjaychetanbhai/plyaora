...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $city_id && $sport_id && $owner_id && $price && $turf_id) {
        $stmt = $conn->prepare("UPDATE turfs SET name=?, city_id=?, sport_id=?, owner_id=?, price=?, status=? WHERE id=?");
        $stmt->bind_param("siiidsi", $name, $city_id, $sport_id, $owner_id, $price, $status, $turf_id);
        $stmt->execute();
    ...
    <form method="POST" action="" class="space-y-6 max-w-2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="owner_id" required class="w-full bg-void/80 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 transition-all shadow-inner appearance-none">
                    <?php while($o = $owners->fetch_assoc()): ?>
                        <option value="<?php echo $o['id']; ?>" <?php echo $o['id'] == $turf['owner_id'] ? 'selected' : ''; ?>><?php echo e($o['name']); ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div>
        ...
        <button type="submit" class="bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white px-6 py-3 rounded-lg text-sm font-semibold tracking-wider transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] mt-4 inline-block active:scale-[0.98]">
            Update Turf Details
        </button>
    </form>
</div>
