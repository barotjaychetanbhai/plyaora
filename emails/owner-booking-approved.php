<?php
function getOwnerBookingApprovedEmail($playerName, $turfName, $date, $slots, $qrUrl = null, $ticketId = null, $token = null) {
    ob_start();
    ?>
    <div style="text-align: center; margin-bottom: 30px;">
        <span style="display: inline-block; padding: 8px 16px; background-color: #10b981; color: #000; border-radius: 8px; font-weight: bold; font-size: 14px; letter-spacing: 1px; text-transform: uppercase;">Booking Approved</span>
    </div>
    
    <h2 style="color: #ffffff; font-size: 20px; margin-bottom: 10px; font-weight: 600;">Hello <?php echo htmlspecialchars($playerName); ?>,</h2>
    <p style="color: #9ca3af; font-size: 15px; margin-bottom: 25px; line-height: 1.5;">Great news! The owner has approved your cash booking request for <strong><?php echo htmlspecialchars($turfName); ?></strong>.</p>
    
    <?php if ($qrUrl): 
        $emailQrUrl = $qrUrl;
        // fallback to remote PNG generator for emails because SVG support is low in mail clients
        if (strpos($qrUrl, '.svg') !== false && $token && $ticketId) {
    ...
    <div style="background-color: #1a1a1f; border-radius: 12px; padding: 25px; margin-bottom: 30px;">
        ...
        <div style="margin-bottom: 5px;">
            <p style="color: #6b7280; font-size: 11px; text-transform: uppercase; margin: 0 0 5px 0;">Confirmed Slots</p>
